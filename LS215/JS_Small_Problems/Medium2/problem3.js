...
function triangle(a, b, c) {
  if ((a + b + c) > 180 || (a + b + c) < 180) return 'invalid';
  if ([a, b, c].filter(angle => angle === 90).length > 1) return 'invalid';
  if ([a, b, c].filter(angle => angle === 90).length === 1) return 'right';
  if ([a, b, c].filter(angle => angle > 90).length === 1) return 'obtuse';
  return 'acute';
}
